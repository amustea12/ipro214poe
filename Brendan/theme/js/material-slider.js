...
(function($) {
  ...
  $.fn.rkmd_rangeSlider = function() {
    ...
    if(self.hasClass('slider-continuous') === true) {
      ...
      sliderContinuous.each(function(i, v) {
        ...
        slider_fill.css('width', ((parseInt(range_val) - parseInt(range.attr('min'))) / (parseInt(range.attr('max')) - parseInt(range.attr('min'))))*100 +'%');
        slider_handle.css('left', ((parseInt(range_val) - parseInt(range.attr('min'))) / (parseInt(range.attr('max')) - parseInt(range.attr('min'))))*100 +'%');
  ...
  function slider_move(parents, newW, sliderW) {
    ...
    range.val((slider_new_val/100*(parseInt(range.attr('max'))-parseInt(range.attr('min')))+parseInt(range.attr('min'))).roundTo(parseInt(range.attr('step')) || 1));

    if(parents.hasClass('slider-discrete') === true) {
      parents.find('.slider-handle span').text((slider_new_val/100*(parseInt(range.attr('max'))-parseInt(range.attr('min')))+parseInt(range.attr('min'))).roundTo(parseInt(range.attr('step')) || 1));
    }
  }
...
Number.prototype.roundTo = Number.prototype.roundTo || function(num) {
    var resto = this%num;
    if (resto <= (num/2)) { 
        return this-resto;
    } else {
